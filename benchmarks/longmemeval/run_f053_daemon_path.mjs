 ...
 *
 * Provenance: copied verbatim from everest-an/Awareness-Market
 * `benchmarks/longmemeval/run_f053_daemon_path.mjs` (2026-09-06). Sole change
 * for this repo's layout: SDK_ROOT resolves to `local/src` (this repo keeps
 * the local package at the repo root; the source repo used `sdks/local/`).
 * The dataset file `longmemeval_s_cleaned.json` is gitignored — download it
 * from https://huggingface.co/datasets/xiaowu0162/longmemeval-cleaned
 * (LongMemEval_S, 500 questions) into this directory before running.
 *
 * Usage:
...
const SDK_ROOT = path.resolve(__dirname, '../../local/src');
